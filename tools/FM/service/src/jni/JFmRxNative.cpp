...
void *entryFunctionForRdsThread(void *data)
{
    ...
    while(!g_stopCommListener)
    {
        ...
        if(pfd.revents == (POLLPRI | POLLIN)) {
            ...
            if (fd < 0) {
                LOGE("Can't open %s", FMRX_COMP_SCAN_SYSFS_ENTRY);
                return NULL;
            }

            ret = write(fd, &cmd, sizeof(char));
            if(ret < 0){
                LOGE("Failed to read Complete Scan results\n");
                return NULL;
            }
            ...
            if(ret < 0){
                LOGE("reading '/dev/radio0' failed %s\n",
                        strerror(ret));
                return NULL;
            }
...
static int nativeJFmRx_CompleteScan(JNIEnv *env, jobject obj, jlong jContextValue)
{
    ...
    if(isThreadCreated == false)
    {
        ...
        if(res <= 0){
            LOGE("Failed to Start Complete Scan\n");
            goto exit;
        }
            ...
            unsigned int * station_data, int channel_count)
{
    ...
    LOGD("nativeJFmRx_CompleteScan_Callback: Entering");
   ...
   if no only then try to attach to the current thread. */
    ...
    if (env->ExceptionOccurred()) {
        LOGE("%s: Calling nativeCb_fmRxRadioText failed", __func__);
        goto CLEANUP;
    }
    ...
    if (env->ExceptionOccurred()) {
            LOGE("nativeJFmRx_CompleteScan_Callback:  ExceptionOccurred");
            goto CLEANUP;
        }
